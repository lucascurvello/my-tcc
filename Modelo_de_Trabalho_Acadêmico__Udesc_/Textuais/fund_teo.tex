...
\begin{itemize}
    ...
    \item IMU de 9 Eixos: Essencial para a estabilização, a Unidade de Medida 
...
seu antecessor apresentado no trabalho \cite{fink2020proprioceptive}.Como um estimador de estados de última geração, 
...
\subsection{Observador de atitude e orientação}
A orientação do robô é processada por uma sofisticada estrutura em cascata, 
...
\subsection{Fusão de sensorer}
O MUSE realiza a fusão das medidas inerciais (IMU) com a odometria das pernas e 
os dados exteroceptivos (câmera/Lidar) por meio de algoritmos baseados em filtros de Kalman. Um diferencial é o desacoplamento entre 
atitude e posição, o que transforma a dinâmica no Filtro de Kalman em um sistema 
linear e invariante no tempo (LTI), garantindo a convergência matemática do filtro.
...
da IMU e da odometria de pernas para manter uma estimativa imediata. Assim que os 
dados da câmera ou Lidar chegam, o sistema aplica correções retrospectivas, 
